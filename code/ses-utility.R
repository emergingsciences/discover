...
ses.pred_item_kfold <- function(dats, mod, n.folds, reps, xnames, ynames, lambda.seq) {
  ...
  # Initialize a list to store actual and predicted values for each repetition
  predictions <- list()
  ...
  for(r in 1:reps) {
    ...
    # Store actuals and residuals values for this repetition
    predictions[[r]] <- data.frame(Actual = as.matrix(apply(dats[, ynames], 1, mean)), Residuals = apply(dats[, ynames] - yhat, 1, mean))

    
    # Factor-level RMSEp
    rmsep <- sqrt(sum((dats[, ynames[1:7]] - yhat[, 1:7])^2) / (length(ynames[1:7]) * nrow(dats)))
    rmsep2 <- sqrt(sum((dats[, ynames[8:10]] - yhat[, 8:10])^2) / (length(ynames[8:10]) * nrow(dats)))
    rmsep3 <- sqrt(sum((dats[, ynames[11:13]] - yhat[, 11:13])^2) / (length(ynames[11:13]) * nrow(dats)))
    rmsep4 <- sqrt(sum((dats[, ynames[14:16]] - yhat[, 14:16])^2) / (length(ynames[14:16]) * nrow(dats)))
    rmsep5 <- sqrt(sum((dats[, ynames[17:18]] - yhat[, 17:18])^2) / (length(ynames[17:18]) * nrow(dats)))
    
    results <- rbind(results, data.frame(model = 1, rmsep = rmsep))
    results <- rbind(results, data.frame(model = 2, rmsep = rmsep2))
    results <- rbind(results, data.frame(model = 3, rmsep = rmsep3))
    results <- rbind(results, data.frame(model = 4, rmsep = rmsep4))
    results <- rbind(results, data.frame(model = 5, rmsep = rmsep5))
  }
  
  return(list(results = as.data.frame(results), predictions = predictions))
}
